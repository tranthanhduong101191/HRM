...
<div class="wrapper">
    ...
    <header class="main-header">
        ...
        <a href="{{asset('/')}}" class="logo">
            ...
            <span class="logo-lg">Tân Quê</span>
        </a>
                        ...
                        <li class="dropdown user user-menu">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li class="user-footer">
                                    ...
                                    {{--</div>--}}
                                    <div class="pull-left">
                                        <a href="{{asset('config/user/changepassword?id='.auth()->user()->id)}}" class="btn btn-info btn-flat">Đổi mật khẩu</a>
                                    </div>
    ...
    <!-- <aside class="main-sidebar">-->
        ...
        <!-- <section class="sidebar" id="leftside" style="padding-top: 20px">
            <div class="col-md-12">
                @if(auth()->check())
                <div class="panel panel-primary">
                    <div class="panel-heading"><h3 class="panel-title">Chấm công mới nhất</h3></div>
                    <div class="panel-body">

                        <?php
                        $newChamcong = \App\Checkinout::where('MaChamCong', auth()->user()->uid)->orderBy('GioCham', 'DESC')->take('5')->get();
                        ?>
                        <ul>
                            @foreach($newChamcong as $cham)
                                <li>{{\Carbon\Carbon::parse($cham->GioCham)->format('H:i:s')}}</li>
                            @endforeach
                        </ul>
                    </div>
                </div>
                <div class="panel panel-info">
                    <div class="panel-heading"><h3 class="panel-title">Lịch làm việc</h3></div>
                    <div class="panel-body">
                        <?php
                        $lich    =   \App\Chamcong::where('user_id', auth()->user()->id)->where('ngay', '>', \Carbon\Carbon::now()->toDateString())->take(5)->get();
                        ?>
                        <table class="table table-striped table-condensed table-calendar">
                            @foreach($lich as $l)
                                <tr>
                                    <th>{{\Carbon\Carbon::parse($l->ngay)->format('d/m')}}</th>
                                    <td>{{$l->ca_name}}</td>
                                </tr>
                            @endforeach
                        </table>
